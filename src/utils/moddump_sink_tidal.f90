...
module moddump
  ...
  !
   ...
   r0    = a0*(1 -ecc*ecc)/(1+ecc*cos(nu))
   x0    = r0*cos(nu)
   y0    = r0*sin(nu)
   vx0   = -sqrt(U/(1-ecc*ecc)/a0) * sin(nu)
   vy0   = sqrt(U/(1-ecc*ecc)/a0) * (cos(nu)+ecc)
  ...
  ! -- create black hole as sink particle
   ...
   write(*,'(a,Es12.5,a)') ' Initial SMA         = ',a0,' R_sun'
   ...
   write(*,'(a,Es12.5,a)') ' true anomaly        = ',nu, ' arc'
  ...
  subroutine write_setupfile(filename)
   ...
   call write_inopt(nu,    'nu',    'true anomaly',                                        iunit)
   ...
   call write_inopt(a0,    'a0',    'initial SMA',                                         iunit)
  ...
  subroutine read_setupfile(filename,ierr)
   ...
   call read_inopt(nu,    'nu',    db,min=0.,errcount=nerr)
